...
def get_total_order_amount(order: List[PurchaseItem]) -> float:
    """
    Calculate the total cost of all the items ordered.
    """
    return sum(item.price for item in order)

def get_service_charge(order: List[PurchaseItem]) -> float:
    """
    Calculate the service charge based on the order amount.
    For every Rs. 100, the service charge amount should increase by 1% of order amount,
    up to a max of 20%.
    """
